...
public class UpdateScienceDirectFrame extends FrameClass{
	...
	private UpdateScienceDirectFrame(){
		initControls();
		initLayout();
	}
	
	public static final UpdateScienceDirectFrame getFrame(){
		UpdateScienceDirectFrame frame = new UpdateScienceDirectFrame();
	...
	private void initControls(){
		...
		moveCB = new JComboBox<String>(new String[]{"自动更新ScienceDirect文章"});
		...
		publicationText = new JTextField("2015", 10); // 把publicationText当做startEntrez用
		cookieText =  new JTextField("2015",10); // 把cookieText当做endEntrez用
		excelText =  new JTextField("science direct publications.xlsx",15); // excel 文件名 label
		intervalText =  new JTextField("1",3); // 间隔时间 label， ScienceDirect貌似不会封ip，设置1秒间隔。
	}
